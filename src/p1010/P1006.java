...
public class P1006 {
	 ...
	 * dh=30h+0.5m+s/120
	 * dsm=ds-dm=5.9s-6m
	 * dsh=ds-dh=719/120*s-0.5m-30h
	 * dmh=dm-dh=5.5m+11/120*s-30h
	 * D<=|as+b|<=360-D
	 * @param thres_deg
	 * @return
	 */
		...
		for(int i=0;i<a.length/2;i++)
			for(int j=0;j<b.length/2;j++)
			{
				if(a[2*i]<a[2*i+1]&&b[2*j]<b[2*j+1]){
					...
					if(d[4*i+2*j+1]<=d[4*i+2*j]){ // invalid interval
						d[4*i+2*j+1]=0;d[4*i+2*j]=0;
					}
				}
			}		
		...
		for(int i=0;i<d.length/2;i++)
			for(int j=0;j<c.length/2;j++)
				if(d[2*i]<d[2*i+1]&&c[2*j]<c[2*j+1]){
					...
					if(e[4*i+2*j+1]<=e[4*i+2*j]){ // invalid interval 
						e[4*i+2*j+1]=0;e[4*i+2*j]=0;
					}
				}		
		return e;
	}
